...
module.exports = function bcit(num) {
	num = Number(num)
	if (num ==  Infinity) return { sign: "0", exponent: "1111", mantissa: "00000", loss: null }
	if (num == -Infinity) return { sign: "1", exponent: "1111", mantissa: "00000", loss: null }
	let float = { sign: "0", exponent: "0000", mantissa: "00000", loss: null }
	if (num < 0) {
		float.sign = "1"
		num = -num
	}
	...
	// if (data.repeat) {
	// 	// repeat this addition more times for greater precision
	// 	// though in the end this isn't really necessary
	// 	bits += data.frac
	// }
	...
	// 	let value = expo - mantissa.length - i - 1
	// 	console.log(value)
	// 	if (loss[i]) {
